...
This document describes the core architecture, its design rationale, usage patterns, and all concrete implementations within the project.
...
└──────────────────────────────┘  └────────────────────────────────────────────────┘
       ...
       │  └──────────────────────────────┘     │  │ UProjectileRadiusProperty                │
       │                                       │  │   Radius: float                          │
       │  Used by: AWeaponBase (extends AItem) │  │ UProjectileOnHitBehaviorProperty         │
       │                                       │  │   OnHitBehavior: EProjectileOnHitBehavior│
...
Actors declare mandatory properties via `GetMandatoryProperties_Implementation()`:

```cpp
// AItem requires UDurabilityProperty
inline virtual TSet<TSubclassOf<ULyokoProperty>> 
GetMandatoryProperties_Implementation() const override
{
    return { UDurabilityProperty::StaticClass() };
}

// AWeaponBase requires UAttackCooldownProperty on top of AItem's requirements
inline virtual TSet<TSubclassOf<ULyokoProperty>> 
GetMandatoryProperties_Implementation() const override
{
...
The schema is validated against these requirements when assigned, and at runtime via `IsItemDefinitionValid()` / `IsProjectileSchemaValid()`.
...
```
AImpulseProjectile (extends AProjectileBase)
...
public:
    ...
    inline PropertyType *GetProperty() const
    {
        ...
        return MySchema ? MySchema->GetProperty<PropertyType>() : nullptr;
    }
    ...
    bool ValidateSchema() const
    {
        ...
        if (!MySchema->IsA(GetMinimumSchemaClass())) return false;
        return MySchema->HasProperties(GetMandatoryProperties());
    }

protected:
    UFUNCTION(BlueprintNativeEvent)
    TSubclassOf<UMySchema> GetMinimumSchemaClass() const;
...
- `AProjectileBase::GetMinimumProjectileSchemaClass_Implementation()` returns `UProjectileSchema::StaticClass()` (not a derived class), allowing any schema to be assigned
- `AImpulseProjectile::GetMinimumProjectileSchemaClass_Implementation()` also returns `UProjectileSchema::StaticClass()` — impulse data is optional via composition, not enforced by schema type
...
3. **Runtime validation** — mandatory properties enforced at assignment and runtime
4. **Editor validation** — automatic checks for key/value consistency and property data validity
5. **Typed access** — `GetProperty<T>()` provides compile-time checked property access
6. **Extensibility** — new property types can be added without modifying any schema
7. **Blueprint support** — all schemas and properties are BlueprintType/Blueprintable
